...
// Warning (TransparentUnionAttr): 71: could not parse &{57698448 {/usr/include/stdlib.h 71 0 35 0 } []}
...
// Warning (FullComment): 31: could not parse &{61225872 {/home/lepricon/go/src/github.com/Konstantin8105/History_frame3DD/src/microstran/vec3.h 31 0 1 45 } [0xc4213440c0]}
...
// Warning (FullComment): 1: could not parse &{61235264 {/home/lepricon/go/src/github.com/Konstantin8105/History_frame3DD/src/NRutil.h 1 4 4 37 } [0xc4202e5b00]}
...
func main() {
	...
	var // Warning (FullComment): 1: could not parse &{61235264 {/home/lepricon/go/src/github.com/Konstantin8105/History_frame3DD/src/NRutil.h 1 4 4 37 } [0xc4202e5b00]}
	...
	if anlyz != 0 {
		rand.Seed(int64(uint32(int32((int32((noarch.Time(nil))))))))
		{
			for lc = 1; lc <= nL; func() int {
				...
				{
					for i = 1; i <= nE; func() int {
						i += 1
						return i
					}() {
						for j = 1; j <= 12; func() int {
							j += 1
							return j
						}() {
							Q[i][j] = 0
...
func assemble_K(K [][]float64, DoF int, nE int, xyz []vec3, r []float32, L []float64, Le []float64, N1 []int, N2 []int, Ax []float32, Asy []float32, Asz []float32, Jx []float32, Iy []float32, Iz []float32, E []float32, G []float32, p []float32, shear int, geom int, Q [][]float64, debug int) {
	...
	}
	{
		for i = 1; i <= nE; func() int {
			...
			{
				for l = 1; l <= 12; func() int {
					l += 1
					return l
				}() {
					ii = ind[l][i]
					{
						for ll = 1; ll <= 12; func() int {
							ll += 1
							return ll
						}() {
							jj = ind[ll][i]
...
func static_condensation(A [][]float64, N int, c []int, n int, Ac [][]float64, verbose int) {
	...
	k = 1
	{
		for i = 1; i <= N; func() int {
			i += 1
			return i
		}() {
			ok = 1
			{
				for j = 1; j <= n; func() int {
					j += 1
					return j
				}() {
					if c[j] == i {
	...
	{
		for i = 1; i <= N-n; func() int {
			i += 1
			return i
		}() {
			{
				for j = i; j <= N-n; func() int {
					j += 1
					return j
				}() {
					ri = r[i]
	...
	}
	{
		for lc = 1; lc <= nL; func() int {
			...
			if nU[lc] > 0 {
				...
				{
					for n = 1; n <= nU[lc]; func() int {
						n += 1
						return n
					}() {
						noarch.Fprintf(fp, []byte(" %5d\x00"), int((U[lc][n][1])))
						{
							for i = 2; i <= 4; func() int {
								i += 1
								return i
							}() {
			...
			if nW[lc] > 0 {
				...
				{
					for n = 1; n <= nW[lc]; func() int {
						n += 1
						return n
					}() {
						noarch.Fprintf(fp, []byte(" %5d\x00"), int((W[lc][n][1])))
						{
							for i = 2; i <= 5; func() int {
								i += 1
								return i
							}() {
		...
		for m = 1; m <= nE; func() int {
			...
			}()
			{
				for i = 1; i <= nx; func() int {
					...
					{
						for n = 1; n <= 10*nE && cW < nW; func() int {
							n += 1
							return n
						}() {
							if int(W[n][1]) == m {
					...
					tx_ = tx
					{
						for n = 1; n <= 10*nE && cP < nP; func() int {
							n += 1
							return n
						}() {
							if int(P[n][1]) == m {
...
func animate(IN_file []byte, meshpath []byte, modepath []byte, plotpath []byte, title []byte, anim []int, nN int, nE int, DoF int, nM int, xyz []vec3, L []float64, p []float32, J1 []int, J2 []int, f []float64, V [][]float64, exagg_modal float64, D3_flag int, pan float32, scale float32) {
	...
	i = 1
	{
		for (func() int {
			...
			{
				for c = 1; c <= CYCLES; func() int {
					c += 1
					return c
				}() {
					{
						for fr = 0; float32(fr) <= frames; func() int {
							fr += 1
							return fr
						}() {
							func() int {
...
func subspace(K [][]float64, M [][]float64, n int, m int, w []float64, V [][]float64, tol float64, shift float64, iter []int, ok []int, verbose int) {
	...
	{
		for i = 1; i <= m; func() int {
			i += 1
			return i
		}() {
			idx[i] = 0
			{
				for j = i; j <= m; func() int {
					j += 1
					return j
				}() {
					func() float64 {
...
func jacobi(K [][]float64, M [][]float64, E []float64, V [][]float64, n int) {
	...
	}
	{
		for iter = 1; iter <= 2*n; func() int {
			...
			tol = 0
			{
				for d = 1; d <= (n - 1); func() int {
					d += 1
					return d
				}() {
					{
						for i = 1; i <= (n - d); func() int {
							i += 1
							return i
						}() {
							j = i + d
	...
	if reduce != 0 {
		{
			for k = 1; k <= n; func() int {
				...
				{
					for i = k + 1; i <= n; func() int {
						i += 1
						return i
					}() {
						A[i][k] /= pivot
						{
							for j = k + 1; j <= n; func() int {
								j += 1
								return j
							}() {
								A[i][j] -= A[i][k] * A[k][j]
	...
	if reduce != 0 {
		{
			for j = 1; j <= n; func() int {
				...
				{
					for i = m; i < j; func() int {
						i += 1
						return i
					}() {
						A[j][i] = A[i][j]
						{
							for k = m; k < i; func() int {
								k += 1
								return k
							}() {
								A[j][i] -= A[j][k] * A[i][k]
 ...
 */func prodAB(A [][]float64, B [][]float64, C [][]float64, I int, J int, K int) {
	...
	var k int
	{
		for i = 1; i <= I; func() int {
			i += 1
			return i
		}() {
			for k = 1; k <= K; func() int {
				k += 1
				return k
			}() {
				C[i][k] = 0
...
func scanFile(fp *noarch.File, head_lines int, start_chnl int, stop_chnl int) int {
	...
	}
	{
		for {
			{
				for chn = start_chnl; chn <= stop_chnl; func() int {
					chn += 1
					return chn
				}() {
 ...
 */func showProgress(i int, n int, count int) {
	...
	{
		for k = 1; k <= line_length+13; func() int {
			k += 1
			return k
		}() {
	...
	{
		for k = j; k < line_length; func() int {
			k += 1
			return k
		}() {
			noarch.Fprintf(stderr, []byte(" \x00"))
 ...
 */func save_matrix(filename []byte, A [][]float32, ml int, mh int, nl int, nh int, transpose int, mode []byte) {
	...
	if transpose != 0 {
		{
			for j = nl; j <= nh; func() int {
				j += 1
				return j
			}() {
				{
					for i = ml; i <= mh; func() int {
						i += 1
						return i
					}() {
						if A[i][j] != float32(0) {
